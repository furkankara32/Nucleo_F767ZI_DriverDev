...
static void USART3_SendHexNibble(uint8_t value)
{
	uint8_t ch;
...
static uint8_t HexCharToNibble(uint8_t ch, uint8_t *nibble)
{
	if ((ch >= '0') && (ch <= '9'))
	{
		*nibble = ch - '0';
...
static uint8_t AsciiHexToByteParser(uint8_t ascii_ch, uint8_t *out_byte)
{
	static uint8_t high_nibble = 0U;
	static uint8_t has_high_nibble = 0U;
	 ...
	 * This allows both:
	 * AA050148454C4C4FEC
	 * AA 05 01 48 45 4C 4C 4F EC
	 */
	if ((ascii_ch == ' ') || (ascii_ch == '\r') || (ascii_ch == '\n') || (ascii_ch == '\t'))
	{
		return 0U;
	}

	if (HexCharToNibble(ascii_ch, &nibble) == 0U)
	{
		has_high_nibble = 0U;
		return 0U;
	}

	if (has_high_nibble == 0U)
	{
		high_nibble = nibble;
		has_high_nibble = 1U;
		return 0U;
	...
	*out_byte = (uint8_t)((high_nibble << 4U) | nibble);
	has_high_nibble = 0U;
...
void EX011_USART3_PacketParser(void)
{
	...
	while (1)
	{
		...
		while (USART_RxReadByte(&USART3Handle, &rx_byte) == USART_STATUS_OK)
		{
			...
			if (AsciiHexToByteParser(rx_byte, &parsed_byte) == 1U)
			{
