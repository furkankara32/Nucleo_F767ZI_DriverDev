/*
 * ex028_i2c1_it_write_read_i2c2_slave_test.c
 *
 ...
 * ex028_i2c1_it_write_read_i2c2_slave_test.c
 *
 * I2C1 interrupt master write-read test with I2C2 polling slave.
 *
 * Flow:
 * I2C1 master writes 1 register byte to I2C2 slave.
 * I2C1 master generates repeated START.
 * I2C1 master reads 5 bytes from I2C2 slave.
 *
 * Wiring:
...
#if ACTIVE_EXAMPLE == EXAMPLE_I2C1_IT_WRITE_READ_I2C2_SLAVE_TEST
...
static uint8_t g_i2c1_reg_addr = 0x75U;

static uint8_t g_i2c2_response_buffer[] =
{
    0xA1U, 0xB2U, 0xC3U, 0xD4U, 0xE5U
};

static uint8_t g_i2c1_rx_buffer[sizeof(g_i2c2_response_buffer)];
...
static volatile uint32_t g_i2c1_wr_rd_complete_count = 0U;
...
static volatile uint32_t g_i2c2_rxne_count = 0U;
...
static volatile uint8_t  g_i2c2_received_reg = 0U;
...
static void I2C2_SlavePollWriteRead(void)
{
    uint32_t isr;
    ...
    /*
     * Address matched.
     * In this test we expect two address matches:
     * 1) write phase
     * 2) repeated-start read phase
     */
    ...
    /*
     * Write phase:
     * Master sends register address byte.
     */
    if (isr & I2C_FLAG_RXNE)
    {
        g_i2c2_received_reg = (uint8_t)I2C2->I2C_RXDR;
        g_i2c2_rxne_count++;
    }
    ...
    {
        if (g_i2c2_tx_index < sizeof(g_i2c2_response_buffer))
        {
            I2C2->I2C_TXDR = g_i2c2_response_buffer[g_i2c2_tx_index];
            g_i2c2_tx_index++;
...
void EX028_I2C1_IT_WriteRead_I2C2_Slave_Test(void)
{
     ...
     * repeated-start,
     * read 5 bytes from I2C2 slave.
     */
    status = I2C_MasterWriteReadIT(&I2C1Handle,
                                   I2C2_SLAVE_ADDR,
                                   I2C_ADDR_MODE_7BIT,
                                   &g_i2c1_reg_addr,
                                   1U,
    ...
    {
        /*
         * Poll I2C2 slave write-read behavior fast.
         */
        I2C2_SlavePollWriteRead();

        if ((g_i2c1_transfer_done == 1U) &&
            (g_i2c2_slave_done == 1U))
        {
            g_i2c1_compare_ok = I2C_CompareBuffers(g_i2c2_response_buffer,
                                                   g_i2c1_rx_buffer,
                                                   sizeof(g_i2c2_response_buffer));
        ...
        if (g_test_done == 1U)
        {
             ...
             * g_i2c1_wr_rd_complete_count = 1
             * g_i2c1_nack_count = 0
             * g_i2c1_error_count = 0
             *
             * g_i2c2_addr_count = 2
             * g_i2c2_rxne_count = 1
             * g_i2c2_received_reg = 0x75
             * g_i2c2_txis_data_count = 5
             * g_i2c2_stop_count = 1
             *
             * g_i2c1_compare_ok = 1
                                  ...
                                  uint8_t Event)
{
    if (pI2CHandle->pI2Cx == I2C1)
    {
        ...
        if (Event == I2C_EVENT_WR_RD_CMPLT)
        {
            g_i2c1_wr_rd_complete_count++;
...
#endif /* ACTIVE_EXAMPLE == EXAMPLE_I2C1_IT_WRITE_READ_I2C2_SLAVE_TEST */
