...
#if ACTIVE_EXAMPLE == EXAMPLE_USART3_TX
...
static uint8_t USART3_Inits(void)
{
	...
	status = USART_Init(&USART3Handle);

	return status;
}

void EX007_USART3_Tx(void)
{
	uint8_t msg[] = "USART3 TX timeout test from NUCLEO-F767ZI\r\n";
	uint8_t status;

	SysTick_Init(16000000U);

	USART3_GPIOInits();
	if(USART3_Inits() != USART_STATUS_OK)
	{
		while(1);
	}
	//USART_DeInit(USART3);
	while(1)
		{
			status = USART_SendDataWithTimeout(&USART3Handle, msg, sizeof(msg) - 1U, 100);
...
#endif /* ACTIVE_EXAMPLE == EXAMPLE_USART3_TX */
